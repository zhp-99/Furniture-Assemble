...
def pre_push(env, start_ee_states, env_states):
    ...
    part1_forward = part1_angles[0].item()
    print("Angles: ", part1_angles/np.pi)
    print("Forward: ", part1_forward/np.pi)

    target_ori_1 = rot_mat_tensor(0, np.pi/2+np.pi/3, 0, env.device)
    target_ori_1 = rot_mat_tensor(0, 0, np.pi/2-part1_forward, env.device) @ target_ori_1
    target_pos_1 = part1_pos
    target_pos_1[0] -= 0.25*np.sin(part1_forward)
    target_pos_1[1] -= 0.25*np.cos(part1_forward)
    target_pos_1[2] += 0.000
